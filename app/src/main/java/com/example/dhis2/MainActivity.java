...
import static com.example.dhis2.logout.logOut;
...
public class MainActivity extends AppCompatActivity  implements NavigationView.OnNavigationItemSelectedListener{
    ...
    private User getUser() {
        return setup.d2().userModule().user().blockingGet();
    }
    @Override
    ...
    private void updateSyncDataAndButtons() {
        ...
        int programCount = synchelp.programCount();
        int dataSetCount = synchelp.dataSetCount();
        int trackedEntityInstanceCount = synchelp.trackedEntityInstanceCount();
        int singleEventCount = synchelp.singleEventCount();
        int dataValueCount = synchelp.dataValueCount();
        int noteModule = synchelp.noteModule();
        int organisationUnitModel = synchelp.organisationUnitModel();
        int periodModule = synchelp.periodModule();
        ...
        TextView downloadedNoteValue = findViewById(R.id.noteModule);
        TextView downloadedorganisationUnitModel = findViewById(R.id.orgUnitModel);
        TextView downloadedperiodModule = findViewById(R.id.periodModule);
        ...
        downloadedNoteValue.setText(MessageFormat.format("{0}", noteModule));
        downloadedperiodModule.setText(MessageFormat.format("{0}", periodModule));
        downloadedorganisationUnitModel.setText(MessageFormat.format("{0}",organisationUnitModel));
    ...
    private Observable<D2Progress> downloadMetadata() {
        return setup.d2().metadataModule().download();
    }
    ...
    private Observable<D2Progress> downloadTrackedEntityInstances() {
        return setup.d2().trackedEntityModule().trackedEntityInstanceDownloader()
                .limit(10).limitByOrgunit(false).limitByProgram(false).download();
    }
    private Observable<D2Progress> downloadAggregatedData() {
        return setup.d2().aggregatedModule().data().download();
    }
    private void wipeData() {
        compositeDisposable.add(
                Observable.fromCallable(() -> setup.d2().wipeModule().wipeData())
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .doOnError(Throwable::printStackTrace)
                        .doOnComplete(this::setSyncingFinished)
